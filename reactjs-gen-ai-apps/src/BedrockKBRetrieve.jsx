...
import {
  getBedrockKnowledgeBaseRetriever,
...
export default ({ llm }) => {
  ...
  const handleLLMNewToken = (token) => {
    ...
    if (token === "") {
      setLLMResponse((msg) => {
        setChat((prev) => [
          ...prev,
          { text: msg, sender: "bot", name: "Demo Bot" },
        ]);
        return "";
      });
      setLoading(false);
  ...
  const sendText = async () => {
    ...
    setChat((prev) => [
      ...prev,
      { text: value, sender: "user", name: "Demo User" },
    ]);
    const retriever = await getBedrockKnowledgeBaseRetriever(currentKb.value);
    const chain = await getConversationalRetrievalQAChain(
      llm,
      retriever,
      memory
    );
